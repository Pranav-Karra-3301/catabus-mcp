...
from catabus_mcp.ingest.realtime_poll import RealtimeData, ServiceAlert, TripUpdate, VehiclePosition
from catabus_mcp.ingest.static_loader import GTFSData, Route, Stop, StopTime, Trip
...
def mock_gtfs_data():
    ...
    data.trips = {
        "BL_001": Trip(
            trip_id="BL_001", route_id="BL", service_id="WEEKDAY", trip_headsign="Blue Loop"
        )
    }
...
def mock_realtime_data():
    ...
    data.last_vehicle_update = datetime.now(UTC)
    data.last_trip_update = datetime.now(UTC)
    data.last_alert_update = datetime.now(UTC)
...
class TestNextArrivals:
    ...
    async def test_get_arrivals_with_delay(self, client):
        ...
        with patch("catabus_mcp.tools.next_arrivals.datetime") as mock_dt:
            ...
            mock_dt.now.return_value = datetime(2024, 1, 1, 8, 55, 0, tzinfo=UTC)
            mock_dt.combine.return_value = datetime(2024, 1, 1, 9, 0, 0, tzinfo=UTC)
            mock_dt.fromtimestamp = datetime.fromtimestamp

            result = await client.call_tool(
                "next_arrivals_tool", {"stop_id": "CURTIN_BJC", "horizon_minutes": 60}
            )
            arrivals = result
    ...
    async def test_get_arrivals_invalid_stop(self, client):
        ...
        result = await client.call_tool(
            "next_arrivals_tool", {"stop_id": "INVALID_STOP", "horizon_minutes": 30}
        )
        arrivals = result
    ...
    async def test_get_arrivals_custom_horizon(self, client):
        ...
        result = await client.call_tool(
            "next_arrivals_tool", {"stop_id": "HUB", "horizon_minutes": 120}
        )
        arrivals = result
    ...
    async def test_tool_with_invalid_params(self, client):
        ...
        with pytest.raises(Exception):
            await client.call_tool(
                "next_arrivals_tool",
                {"stop_id": "HUB", "horizon_minutes": "invalid"},  # Should be int
            )
...
class TestIntegrationScenarios:
    ...
    async def test_find_stop_and_get_arrivals(self, client):
        ...
        arrivals = await client.call_tool(
            "next_arrivals_tool", {"stop_id": stop["stop_id"], "horizon_minutes": 60}
        )
