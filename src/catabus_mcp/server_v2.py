"""FastMCP server for CATA bus data - v2 following documentation."""
...
async def ensure_initialized():
    ...
    if not initialized:
        logger.info("Initializing GTFS data...")
        gtfs_data = await static_loader.load_feed()
        await realtime_poller.start()
        initialized = True
        logger.info("GTFS data initialized")
...
# Main entry point - required for FastMCP deployment
